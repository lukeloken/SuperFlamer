...
ConvertGasesSuperFlame=function(correctdata, Elevation){
  ...
  if(length(intersect(names(correctdata), c('CO2', 'CH4')))>=1 ){
    ...
    convertdata$CH4Sat<-as.numeric(CH4uM/CH4getsat*100)
    
    convertdata$CH4uM_hyd<-as.numeric(convertdata$CH4_hyd*CH4kh/Pressure)
    convertdata$CH4Sat_hyd<-as.numeric(CH4uM_hyd/CH4getsat*100)
    
    convertdata$CH4uM_tau<-as.numeric(convertdata$CH4_tau*CH4kh/Pressure)
    convertdata$CH4Sat_tau<-as.numeric(CH4uM_tau/CH4getsat*100)
    ...
    convertdata$CO2Sat<-as.numeric(CO2uM/CO2getsat*100)
    
    convertdata$CO2uM_hyd<-as.numeric(convertdata$CO2_hyd*CO2kh/Pressure)
    convertdata$CO2Sat_hyd<-as.numeric(CO2uM_hyd/CO2getsat*100)
    
    convertdata$CO2uM_tau<-as.numeric(convertdata$CO2_tau*CO2kh/Pressure)
    convertdata$CO2Sat_tau<-as.numeric(CO2uM_tau/CO2getsat*100)
