...
# points <- readRDS(file.path(flame_path, "Merged_Illinois_Jul_2023_Shapefile_AllData.rds"))
# 
...
snap_points_to_river <- function(points, projection, processed_path, flame_file){
  ...
  points_mc <- st_crop(points, bb_mc)
  ...
  points_ao <- st_crop(points, bb_ao)
  ...
  points_fl <- st_crop(points, bb_fl)
  ...
  points_tribs <- st_crop(points, bb_tribs)
