...
MergeChemistry <- function(home_path, 
  ...
  UCD_xlsx<- chem_files[grepl("DOC", chem_files)]
  ...
  UCD_df <- fread(file = UCD_xlsx[1], fill = TRUE,  skip=0, header=T)
  
  UCD_list <- lapply(UCD_xlsx, fread, skip=0, header=T, fill = TRUE,
                     stringsAsFactors=F, 
  ...
  spplot(raster_cropped)
  # plot(watergrid_predict_subset, add = TRUE)
  spplot(b_data, zcol = "dist_to_land", add = TRUE)
  ...
  watergrid_predict_subset_data@data <- watergrid_predict_subset_data@data %>%
    select(all_of(variables), everything())
  ...
  # ==========================================
  var = variables[12]
  for (var in variables[40:length(variables)]){
    var_number <- which(variables == var)
    
    # Select only variable of interest
    data2 <- b_data %>%
      select(all_of(var), "dist_to_land",  "dist_to_green", 
             "focal_300m_4500m", "focal_300m_1500m", "dist_to_river")
    ...
    if (nrow(data2)>0){
      ...
      raw_values <- pull(select(data3@data, all_of(var)))
      ...
      if (identical(round(min(values), 3), round(max(values),3))==FALSE){
        ...
        print(spplot(watergrid_predict_subset_data, zcol = var, 
                     ...
                     sub = "Prediction using regression Kriging with distance to land",
      ...
      rm(PREDICTOR, RESPONSE, power, 
         GLM, v.glm, est_will, est_range, est_nugget, m.f.glm, 
         SK.GLM, k1, values, 
